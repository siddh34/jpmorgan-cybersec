...
class RolesCache:
    ...
    def get(self, role):
        # Returns the message corresponding to the last invocation of that role, or None if not found
        if role in self.cache:
            # Move the accessed role to the end to mark it as recently used
    ...
    def set(self, role, message):
        if role in self.cache:
            # Update the message and move the role to the end to mark it as recently used
            self.cache.move_to_end(role)
        elif len(self.cache) >= self.capacity:
            # Remove the oldest role (first item in OrderedDict)
            self.cache.popitem(last=False)
        
        # Add or update the role with the new message
